...
class DateLtFieldRule extends AbstractRule
{
    const NAME = 'date_lt_field';

    public static function message(array $conditions = []) : string
    {
        return 'validation.date_lt_field';
    }
    ...
    ) : ?string
    {
        ...
        if (! isset($this->parameters[ 0 ])) {
            throw new LogicException(
                'The `parameters[0]` should be present, and known as `dateLtField`'
            );
        }
        ...
        if (! $theType->date($fieldValue)->isOk([ &$dateLt ])) {
            throw new LogicException(
