...
class DateMaxFieldRule extends AbstractRule
{
    const NAME = 'date_max_field';

    public static function message(array $conditions = []) : string
    {
        return 'validation.date_max_field';
    }
    ...
    ) : ?string
    {
        ...
        if (! isset($this->parameters[ 0 ])) {
            throw new LogicException(
                'The `parameters[0]` should be present, and known as `dateMaxField`'
            );
        }
        ...
        if (! $theType->date($fieldValue)->isOk([ &$dateMax ])) {
            throw new LogicException(
